...
export const ColumnOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Column', 'delta9-digital-blocks-plugin')}>
			<WidthOffsetRangeSlider
				{...generateWidthOffsetRangeSliderConfig({
			...
			<Section icon={icons.alignHorizontalVerticalAlt} label={__('Alignment', 'delta9-digital-blocks-plugin')}>
				<Responsive
					label={__('Horizontal', 'delta9-digital-blocks-plugin')}
				...
				<Responsive
					label={__('Vertical', 'delta9-digital-blocks-plugin')}
			...
			<Section icon={icons.tools} label={__('Advanced', 'delta9-digital-blocks-plugin')} noBottomSpacing>
				<ResponsiveToggleButton
					{...generateResponsiveToggleButtonConfig({
				...
				<Responsive
					label={<IconLabel icon={icons.order} label={__('Order', 'delta9-digital-blocks-plugin')} />}
				...
				>
					{getDefaultBreakpointNames().map((breakpoint, index) => {
						...
						let options = [
							{ label: __('Auto', 'delta9-digital-blocks-plugin'), value: true, icon: icons.automatic },
							{ label: __('Manual', 'delta9-digital-blocks-plugin'), value: false, icon: icons.pointerHand },
						];
						...
						return (
							<Fragment key={index}>
								<Control
									help={
										isAuto
											? __('Follows the order of elements in the editor', 'delta9-digital-blocks-plugin')
											: __('Forces column order, independent of the element order in the editor', 'delta9-digital-blocks-plugin')
									}
									noBottomSpacing
								>
									<div className='es-h-spaced es-gap-3!'>
										...
										<AnimatedContentVisibility showIf={!isAuto}>
											<NumberPicker
												label={__('Position', 'delta9-digital-blocks-plugin')}
								...
								<AnimatedContentVisibility showIf={!isAuto && index === 0} additionalContainerClasses='es-mt-2'>
									<Notification
										...
										subtitle={__('Make sure to set the order all other columns in the same Columns block', 'delta9-digital-blocks-plugin')}
										noBottomSpacing
									/>
								</AnimatedContentVisibility>
							</Fragment>
