...
export const FeaturedContentOptions = ({ attributes, setAttributes }) => {
	...
	const allGenericOption = { label: __('All', 'delta9-digital-blocks-plugin'), value: '_all' };
	...
	return (
		<PanelBody title={__('Featured content', 'delta9-digital-blocks-plugin')}>
			...
			<Section showIf={postTypeOptions.length > 0} icon={icons.filter} label={__('Item source', 'delta9-digital-blocks-plugin')} noBottomSpacing>
				<Control additionalClasses='es-fifty-fifty-h'>
					<Select
						label={__('Kind', 'delta9-digital-blocks-plugin')}
					...
					{featuredContentPostType?.taxonomies?.length > 0 &&
						<Select
							label={__('Taxonomy', 'delta9-digital-blocks-plugin')}
							...
							// placeholder={__('All', 'delta9-digital-blocks-plugin')}
							noBottomSpacing
						/>
					}
				</Control>
					...
					<>
						{!featuredContentUseCurrentTerm &&
							<IconToggle
								...
								label={__('Select terms to show', 'delta9-digital-blocks-plugin')}
						...
						<AnimatedContentVisibility showIf={useSpecificTerms && !featuredContentUseCurrentTerm}>
							<AsyncMultiSelect
								...
								help={__('Newest 30 items are shown, others can be selected by searching. If blank, all items are shown.', 'delta9-digital-blocks-plugin')}
						...
						{!useSpecificTerms &&
							<IconToggle
								...
								label={__("Use current item's terms", 'delta9-digital-blocks-plugin')}
								help={__('Best used with posts', 'delta9-digital-blocks-plugin')}
								inlineHelp
				...
				{!featuredContentTaxonomy &&
					<>
						{!featuredContentExcludeCurrentPost &&
							<IconToggle
								icon={icons.itemSelect}
								label={__('Select items to show', 'delta9-digital-blocks-plugin')}
				...
				{!useSpecificPosts &&
					<IconToggle
						...
						label={__('Exclude current', 'delta9-digital-blocks-plugin')}
						help={__('Best used with posts', 'delta9-digital-blocks-plugin')}
				...
				<IconToggle
					icon={icons.dice}
					label={__('Randomize order', 'delta9-digital-blocks-plugin')}
			...
			<Section label={__('Other', 'delta9-digital-blocks-plugin')} icon={icons.moreH} noBottomSpacing>
				<LoadMoreOptions
					...
					label={__('"Load more" button', 'delta9-digital-blocks-plugin')}
					noBottomSpacing
				/>
			</Section>
		</PanelBody>
