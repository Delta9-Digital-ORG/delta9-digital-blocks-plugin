...
export const FeaturedCategoriesOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Featured categories', 'delta9-digital-blocks-plugin')}>
			<NumberPicker
				icon={icons.itemLimit}
				label={__('Items per row', 'delta9-digital-blocks-plugin')}
			...
			<Section icon={icons.filter} label={__('Item source', 'delta9-digital-blocks-plugin')}>
				<Select
			...
			<Section showIf={featuredCategoriesTaxonomy} noBottomSpacing>
				<IconToggle
					...
					label={__('Manually select terms', 'delta9-digital-blocks-plugin')}
