...
export const TableOfContentsOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Table of contents', 'delta9-digital-blocks-plugin')}>

			<Control icon={icons.headingLevelAlt} label={__('Heading levels to include', 'delta9-digital-blocks-plugin')}>
				{Object.keys(tableOfContentsHeadingLevels).map((level, i, arr) => {
					return (
						<IconToggle
							...
							label={level.replace('h', __('Heading', 'delta9-digital-blocks-plugin') + ' ')}
							onChange={(value) => {
