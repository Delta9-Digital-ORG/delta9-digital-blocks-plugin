					...
					{siteFooterLinks.map((link, i) => {
						return (
							<ReOrderableItem
								...
								title={link?.text?.length > 0 ? link.text : <i>{__('New link', 'delta9-digital-blocks-plugin')}</i>}
								preIcon={
									<div className='es-display-flex'>
										...
										>
											<TextControl
												label={__('Link text', 'delta9-digital-blocks-plugin')}
												value={link?.text}
												onChange={(value) => {
											...
											<LinkInput
												label={__('URL', 'delta9-digital-blocks-plugin')}
												url={link?.url}
