		...
		return (
			<UseToggle {...generateUseToggleConfig(attributes, manifest, 'videoUse')}>
				{hideVideoPicker &&
					<Placeholder
						icon={icons.help}
						label={__('No video... yet', 'delta9-digital-blocks-plugin')}
					>
						{__('Add one in the Block editor', 'delta9-digital-blocks-plugin')}
					</Placeholder>
				}
				...
				{!hideVideoPicker &&
					<MediaPlaceholder
						...
						labels={{ title: __('Add a video', 'delta9-digital-blocks-plugin') }}
						accept={videoAccept}
	...
	return (
		<UseToggle {...generateUseToggleConfig(attributes, manifest, 'videoUse')}>
			<AnimatedContentVisibility showIf={hasPoster && !videoControls}>
				<Notification
					type='warning'
					text={__('Playback controls disabled', 'delta9-digital-blocks-plugin')}
					subtitle={__('Poster image might prevent users from playing the video', 'delta9-digital-blocks-plugin')}
				/>
			</AnimatedContentVisibility>
			...
			<Control additionalClasses='es-display-flex es-content-center'>
				{videoUrl?.length > 0 &&
					...
					>
						{__('Remove video', 'delta9-digital-blocks-plugin')}
					</Button>
				}
			</Control>
			...
			<Section
				...
				label={__('Playback options', 'delta9-digital-blocks-plugin')}
			>
					...
					<div className='es-h-spaced-wrap es-mb-3'>
						...
						{!hideMuteToggle &&
							<IconToggle
								...
								label={__('Mute', 'delta9-digital-blocks-plugin')}
								checked={videoMuted}
				...
				<Collapsable label={__('Advanced', 'delta9-digital-blocks-plugin')} icon={icons.moreH} noBottomSpacing>
					{!hidePosterImagePicker &&
						<Control
							icon={icons.videoPosterImage}
							label={__('Poster image', 'delta9-digital-blocks-plugin')}
							subtitle={__('Visible before the video is played', 'delta9-digital-blocks-plugin')}
						>
							{!hasPoster &&
								<MediaPlaceholder
									labels={{ title: __('Add an image', 'delta9-digital-blocks-plugin') }}
							...
							{hasPoster &&
								<div className='es-h-center es-items-end! es-gap-0!'>
									<img
										alt={__('Video poster image', 'delta9-digital-blocks-plugin')}
										src={videoPoster}
									...
									<Button
										icon={icons.trashAlt}
										label={__('Remove image', 'delta9-digital-blocks-plugin')}
										// eslint-disable-next-line max-len
					...
					{!hidePreloadOptions &&
						<OptionSelector
							label={<IconLabel icon={icons.preload} label={__('Preload', 'delta9-digital-blocks-plugin')} standalone />}
							value={videoPreload}
			...
			<Section
				...
				icon={icons.design}
				label={__('Design & functionality', 'delta9-digital-blocks-plugin')}
				additionalClasses='es-h-spaced-wrap'
			>
				{!hideVideoControlsToggle &&
					<IconToggle
						icon={icons.videoControls}
						label={__('Playback controls', 'delta9-digital-blocks-plugin')}
			...
			<Section showIf={!hideCaptions} icon={icons.a11y} label={__('Accessibility', 'delta9-digital-blocks-plugin')} noBottomSpacing>
				<Repeater
					icon={icons.videoSubtitleAlt}
					label={__('Captions', 'delta9-digital-blocks-plugin')}
					...
					{videoSubtitleTracks.map((item, index) => {
						return (
							<RepeaterItem
								...
								title={
									item?.label
										? sprintf(__('Track %d', 'delta9-digital-blocks-plugin'), index + 1)
										: <i>{__('New track', 'delta9-digital-blocks-plugin')}</i>
								}
								subtitle={item?.label}
							...
							>
								{!videoSubtitleTracks[index].src &&
									<MediaPlaceholder
										accept={['.vtt', 'text/vtt']}
										labels={{ title: __('Track file', 'delta9-digital-blocks-plugin') }}
										onSelect={
											(track) => {
									...
									>
										{__('Upload a VTT file containing captions, subtitles, descriptions or chapters', 'delta9-digital-blocks-plugin')}
									</MediaPlaceholder>
								}
								...
								{videoSubtitleTracks[index].src &&
									<>
										<TextControl
											label={<IconLabel icon={icons.titleGeneric} label={__('Label', 'delta9-digital-blocks-plugin')} />}
											help={__('Shows in the list of available tracks', 'delta9-digital-blocks-plugin')}
										...
										<OptionSelector
											label={<IconLabel icon={icons.optionListAlt} label={__('Type', 'delta9-digital-blocks-plugin')} />}
										...
										<TextControl
											label={<IconLabel icon={icons.flag} label={__('Language code', 'delta9-digital-blocks-plugin')} />}
											help={
												<>
													{__('Should follow IETF (BCP47).', 'delta9-digital-blocks-plugin')}
													{videoSubtitleTracks[index].kind === 'subtitles' && ' ' + __('Required.', 'delta9-digital-blocks-plugin')}
												</>
											}
										...
										<span className='es-display-block es-mb-1'>{__('List of language tags', 'delta9-digital-blocks-plugin')}</span>
										...
										>
											{__('Common languages', 'delta9-digital-blocks-plugin')}
										</Button>
										...
										<Button
											...
											className='es-button-icon-18 es-p-0! es-h-8! es-rounded-0.75!'
										>
											{__('All languages', 'delta9-digital-blocks-plugin')}
										</Button>
									</>
								}
							</RepeaterItem>
				...
				<AnimatedContentVisibility showIf={videoAutoplay && !videoMuted && !videoControls} additionalContainerClasses='es-mt-3'>
					<Notification
						type='warning'
						text={__('Video plays automatically, with sound, and without controls', 'delta9-digital-blocks-plugin')}
						// eslint-disable-next-line max-len
						subtitle={__('This will bother most users and is an accessibility issue. Consider changing some of the options.', 'delta9-digital-blocks-plugin')}
