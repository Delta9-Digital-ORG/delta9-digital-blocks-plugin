...
export const SocialNetworksOptions = (attributes) => {
	...
	const modeOptions = [
		{
			label: __('Link to social networks', 'delta9-digital-blocks-plugin'),
			icon: icons.link,
			value: false,
		},
		{
			label: __('Share to social networks', 'delta9-digital-blocks-plugin'),
			icon: icons.share,
			value: true,
		}
	];

	const pickerSectionTitle = socialNetworksShareMode ? __('Share targets', 'delta9-digital-blocks-plugin') : __('Social networks', 'delta9-digital-blocks-plugin');
	...
	return (
		<UseToggle {...generateUseToggleConfig(attributes, manifest, 'socialNetworksUse')}>
			{!hideModeSelector &&
				<OptionSelector
					icon={icons.gears}
					label={__('Mode', 'delta9-digital-blocks-plugin')}
