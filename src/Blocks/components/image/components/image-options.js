...
export const ImageOptions = (attributes) => {
	...
	if (!hideImagePicker && !imageUrl[breakpoints[0]]) {
		return (
			<UseToggle {...generateUseToggleConfig(attributes, manifest, 'imageUse')}>
				<Control icon={icons.image} label={__('Image', 'delta9-digital-blocks-plugin')} additionalLabelClasses='es-h-spaced' noBottomSpacing>
					<MediaPlaceholder
						labels={{
							title: __('Add an image', 'delta9-digital-blocks-plugin'),
	...
	return (
		<UseToggle {...generateUseToggleConfig(attributes, manifest, 'imageUse')}>
			{!hideImagePicker &&
				<Responsive label={__('Image', 'delta9-digital-blocks-plugin')} icon={icons.image}>
			...
			<Section
				...
				icon={icons.design}
				label={__('Design & layout', 'delta9-digital-blocks-plugin')}
			...
			>
				...
				{!hideFullSizeToggle &&
					<IconToggle
						icon={icons.expandXl}
						label={__('Fill container', 'delta9-digital-blocks-plugin')}
						checked={imageFull}
			...
			<Section
				showIf={!hideAltText}
				icon={icons.a11y}
				label={__('Accessibility', 'delta9-digital-blocks-plugin')}
				noBottomSpacing={!additionalControlsAfterA11y}
			>
				<TextControl
					label={<IconLabel icon={icons.altText} label={__('Alt text', 'delta9-digital-blocks-plugin')} />}
					value={imageAlt}
					onChange={(value) => setAttributes({ [getAttrKey('imageAlt', attributes, manifest)]: value })}
					help={__('Describes the content of the image', 'delta9-digital-blocks-plugin')}
