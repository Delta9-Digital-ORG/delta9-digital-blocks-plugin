...
domReady(() => {
	jQuery('.block-product-count').each(function (){
		...
		function updateAddToCart(quantity, thisElm) {
			...
			// Dispatch custom event so the dropdown-picker can sync.
			var productId = $(addToCartButton).attr('data-product_id');
			if (productId) {
				document.dispatchEvent(new CustomEvent('product-count:change', {
					detail: { quantity: parseInt(quantity, 10), productId: String(productId) }
				}));
			}
		}
	});
